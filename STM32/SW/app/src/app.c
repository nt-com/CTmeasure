...
/* @brief a handle to the general purpose timer peripheral */
static gp_timer_handle_t timer_test;
...
/**
 * @brief Initializes the timer peripheral for continuous measurement and data transmission
 * @param void
 * @return void
 */
static void app_gp_timer_init(void);
...
static void app_gp_timer_init(void) {
	...
	timer_test = gpt_get_handle();

	timer_test.pGPTIMx = TIM3;
	timer_test.timer_config.reload_value = GPT_MAX_RELOAD_VALUE;
	timer_test.timer_config.channel = GPT_CHANNEL_1;
	timer_test.timer_config.irq = GPT_IRQ_ENABLE;
	timer_test.timer_config.timer_frequency = GPT_CNT_TIMER_500;	/* prescale to 500 Hz */

	gpt_init(&timer_test);
	gpt_interrupt_config(IRQ_NO_TIM3, GPT_IRQ_ENABLE);

	gpt_ioctl(&timer_test, GPT_IOCTL_START_COUNTER, NULL);
